...
from s2protocol import protocol36442
...
class StormReplayReader:
    ...
    def __init__(self, replayFile):
        ...
        self.buildStormReplay = protocol36442.decode_replay_header(self.mpq.header['user_data_header']['content'])['m_version']['m_baseBuild']

        try:
#            self.protocol = __import__('s2protocol' + '.protocol%s' % self.buildStormReplay, fromlist=['protocol2'])
            self.protocol = __import__('s2protocol' + '.protocol36442', fromlist=['protocol2'])
        except ImportError:
